...
const Songs = {
    ...
    generate: ( title = '', factor = 1, isUser = false ) => {
        ...
        songValues.released = moment( time ).format( Constants.SCHEDULE_DATE_FORMAT );
    ...
    getQuality: ( isUser, factor ) => {
        ...
        let fame = ( isUser ? Protagonist.get( 'fame' ) / Constants.FAME_PROGRESS_FACTOR : Utils.randInt( Constants.MAX_SONG_FACTOR ) ) / 2;
        let creativity = ( isUser ? Protagonist.get( 'creativity' ) : Utils.randInt( Constants.MAX_SONG_FACTOR ) ) / 2;
        let mentality = ( isUser ? Protagonist.get( 'mentality' ) : Utils.randInt( Constants.MAX_SONG_FACTOR ) ) / 2;
        let radRanking = Utils.randRanking( Constants.MAX_SONG_FACTOR );
